...
### 결제 수단 — MoR(Polar·Lemon Squeezy)은 사용 불가

같은 API를 쓴 외부 사례(조코딩 유튜브 튜토리얼)가 **Polar**로 결제를 붙였으나, Polar 이용약관을 확인한 결과 **실물 상품은 금지 목록 1번 항목**이다. "회사의 주된 상품이 실물 재화라면 이 서비스를 사용해서는 안 된다"고 명시되어 있다. Lemon Squeezy도 동일하게 실물 상품을 금지한다.

해당 사례는 주력이 디지털 PDF고 실물 책이 부가였지만, **우리는 상품 자체가 실물 책**이므로 MoR 결제는 선택지가 아니다. 국내 PG가 필요하다.

- **포트원(PortOne)** 권장 — 여러 PG를 한 SDK로 묶어 연동 기간을 단축하고, 나중에 PG를 교체해도 코드 변경이 최소다. 제작사 어댑터와 같은 격리 구조.
- 실제 병목은 코드가 아니라 **PG 심사**(사업자등록증·통신판매업신고)다. Phase 1 착수 시 심사를 먼저 넣는다.

### 벤더 문서 사본

SweetBook이 AI 에이전트용으로 제공하는 `llms.txt` / `llms-full.txt`를 `docs/vendor/`에 사본으로 둔다. 루트 도메인이 자동 조회 시 403을 반환한 전례가 있어 네트워크에 의존하지 않도록 하고, 스펙 변경을 git diff로 추적하기 위함이다. 갱신 방법은 `docs/vendor/README.md` 참조.
...
SweetBook은 세 가지 `creationType`을 제공한다. 문서·템플릿 카탈로그를 실측 조사한 결과 **`PDF_UPLOAD` + `PHOTOBOOK_A5_SC`** 를 택했다.

| 방식 | 판단 | 근거 |
|---|---|---|
| `TEMPLATE` | 부적합 | 템플릿이 사진 배치용(갤러리·일기·알림장)이라 글 위주 본문을 담을 수 없다 |
| `MIX_COVER_TEMPLATE` | 부적합 | A5 표지 템플릿 8종을 전수 조회한 결과 전부 알림장·일기장·소셜포토북 테마. 파라미터가 `childName`·`schoolName`·`coverPhoto`·`dateRange` 형태이고, 일부는 제목조차 디자인에 고정되어 있어 단행본 표지를 만들 수 없다 |
| **`PDF_UPLOAD`** | **채택** | 표지·내지를 완전히 통제. 검증은 크기·페이지 수만 하므로 텍스트 기반 원고에 제약이 없다 |

판형은 제품 안내 페이지가 **A5 소프트커버를 "단행본 · 매뉴얼 · 일기"** 용도로 명시하고 있고, 4종 중 유일하게 200p까지 지원한다. (A4는 "패밀리 앨범·행사 기념책", 스퀘어북은 "동화책·그림책" 용도로 안내된다.) 네 판형 모두 선택지로 노출하되 A5를 기본값·추천으로 둔다.

확인된 제작 사양: CMYK 4도 오프셋급 디지털 인쇄, PUR 무선제본, 무광 라미네이팅, 1권부터, **제작 5~7 영업일**. 파트너 혜택으로 제작비 지원금 10만원과 초기 단가 할인이 안내되어 있다.

### 클레임 예방 설계

SweetBook API가 검증하는 것은 **PDF 크기와 페이지 수뿐**이다. 실제 클레임을 만드는 요인(저해상도, 재단 잘림, 색 차이, 제본 여백)은 검증되지 않고 문서에도 권장값이 없다. 이 공백을 UI가 메운다.

- **표지 도면 자동 생성** (`/api/publish/cover-guide`) — 판형·페이지 수로 계산한 정확한 치수 SVG. 재단선, 책등 위치·두께, 안전영역을 그려 위저드에 인라인 표시하고 내려받게 한다. 책등은 페이지 수에 따라 변하므로 저자가 직접 계산하면 반드시 틀린다.
- **안전영역 기준을 우리가 정의** — 도련 3mm + 여유 5mm = 가장자리 8mm 안쪽에 글자·얼굴 배치. 내지 제본변은 12mm(PUR 특성). 상수는 `lib/publishing/print-guide.ts`에 있다.
- **보내기 전 확인 체크리스트** — 안전영역·해상도(300dpi)·색 차이 3가지를 저자가 명시적으로 체크해야 업로드 버튼이 열린다. 인쇄는 되돌릴 수 없으므로 암묵적 기대를 명시적 확인으로 바꾼다.
- **사양·기간 사전 고지** — 용지·마감·제본·인쇄 방식과 제작 5~7 영업일을 소개 페이지와 업로드 화면 양쪽에 표기한다.
...
**외부 벤치마크:** 같은 API를 쓴 사례에서 고객 결제 29,000원 / 충전금 차감 13,860원 → **마진 52%** 로 운영했다. 우리의 1권 40%는 그보다 보수적이다. 인상 여지가 있다.

같은 사례의 차감액 13,860원은 `SQUAREBOOK_HC 24p 단가 12,600 × 1.1`과 정확히 일치해 **배송비가 0원**이었다. 우리 계정 견적에는 배송비 3,000원(부가세 포함)이 붙는다. 계정별 조건 차이로 보이므로 **협의 시 배송비 정책을 확인**한다.
...
Live 전환 절차는 문서에 명시되어 있다: 사업 협의 → Business 계정 전환 → **파트너 포털에서 Live 키 발급 + 실제 충전금 충전** → `SWEETBOOK_API_BASE`·`SWEETBOOK_API_KEY` 교체. 코드 변경은 없다.

충전금은 선불이므로 **잔액이 마르면 주문이 402로 막힌다.** 운영 전에 잔액 알림 기준을 정해둔다 (`GET /credits`로 조회 가능). 파트너 혜택으로 제작비 지원금 10만원이 안내되어 있다.

3. 배송비 정책 — 계정별로 다를 수 있음 (위 가격 정책 항목 참조)
